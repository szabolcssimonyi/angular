import {
  ...
  NgZone, OnDestroy, AfterViewInit, ChangeDetectorRef
} from '@angular/core';
import { ScrollPanel, MenuItem } from 'primeng/primeng';
...
export class DashboardComponent extends ComponentBase implements AfterViewInit, OnDestroy, OnInit {
  ...
  ngOnInit() {
    ...
    this.subscriptions.push(this.pageService.headerText.subscribe(text => {
      if (!Boolean(text)) {
        return;
      }
      this.headerText = this.translateService.instant(text);
      this.subscriptions.push(this.translateService.onLangChange
        .subscribe(() => this.headerText = this.translateService.instant(text)));
